...
func TestDailyConsumption(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"usage_point_id":"12345678901234","quality":"BRUT","interval_reading":[{"date":"2026-07-01","value":"1250"}]}`))
	}))
	defer server.Close()
	...
	result, err := client.DailyConsumption(context.Background(), date(2026, 7, 1), date(2026, 7, 2))
	if err != nil {
		t.Fatal(err)
	}
	if len(result.IntervalReading) != 1 || result.IntervalReading[0].Value != "1250" {
		t.Fatalf("réponse inattendue: %#v", result)
	}
}

func TestConsumptionLoadCurve(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	result, err := client.ConsumptionLoadCurve(context.Background(), date(2026, 7, 1), date(2026, 7, 2))
	if err != nil {
		t.Fatal(err)
	}
